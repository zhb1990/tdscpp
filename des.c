...
static uint32_t f(const DES_KEY * key, uint32_t r, const unsigned char *subkey);
...
{
	const unsigned char *ib;	/* ptr to input block */
	unsigned char *ob;	/* ptr to output block */
...
static uint32_t
f(const DES_KEY * key, uint32_t r, const unsigned char *subkey)
{
	const uint32_t *spp;
	uint32_t rval, rt;
	int er;
